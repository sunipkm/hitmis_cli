 ...
 * @brief Implementation of Andor USB CameraUnit interfaces
 * @version 0.1
 * @date 2022-01-24
 * 
    ...
    : binningX_(1), binningY_(1), cancelCapture_(true), CCDHeight_(0), CCDWidth_(0), exposure_(0), lastError_(DRV_SUCCESS), m_initializationOK(false), requestShutterOpen_(true), shutterDelayInMs_(shutterDelayInMs)
{
    ...
    HasError(GetHeadModel(cam_name), __LINE__);
    ...
    //     return;
    // }
    if (HasError(SetVSSpeed(0), __LINE__))
    {
        return;
    }
    ...
    if (nAD == 1)
    {
        ...
        if (readOutSpeed > nHSspeeds - 2)
            readOutSpeed = nHSspeeds - 2;
        if (HasError(SetHSSpeed(0, readOutSpeed), __LINE__))
        {
            return;
        }
    }
...
CImageData CCameraUnit_ANDORUSB::CaptureImage(long int &retryCount)
{
    ...
    { // allocate the buffer
        ...
        retVal = buffer;
        retVal.SetImageMetadata(exposure_, binningX_, binningY_, GetTemperature(), exposure_start, CameraName());
    }
